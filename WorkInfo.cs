...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过' and reconmend='是' order by recommendTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public IEnumerable<Model.WorkInfoNo> GetMyPage(Model.WorkMyPage page)
        {
            using(IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select* from a where num between (@pageInsex-1)*@pageSize+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.WorkInfoNo>(sql, page);
            }
        }
        ...
        public int Update(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," +
